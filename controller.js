...
const getUtilisateurById = (req, res) => {
  // const idutilisateur = parseInt(req.params.idutilisateur); //to pass the id from the URL
...
const addUtilisateur = async (req, res) => {
  ...
  const isExist = await pool.query(queries.checkEmailExists, [email]);
  if (isExist.rows.length) {
    res.send("email already exists.");
  } else {
    //add Utilisateur to db
    pool.query(
      queries.addUtilisateur,
...
const getClientavis = (req, res) => {
  pool.query(queries.getClientavis, (error, results) => {
    if (error) throw error;
    res.status(200).json(results.rows);
  });
};

const getClientavisbyID = (req, res) => {
  const idutilisateur = parseInt(req.params.idutilisateur);
  // const idsociete = parseInt(req.body);
  pool.query(queries.getClientavisbyID, [idutilisateur], (error, results) => {
    if (error) throw error;
    res.status(200).json(results.rows);
  });
};
...
const obj1 = {
  reponse1: "email et mot de passe correcte",
  reponse2: "email ou mot de passe incorrecte",
};

const getloginclient = async (req, res) => {
  const { email, password } = req.body;
  // check if email and password  exists
  const isExist = await pool.query(queries.checkloginclient, [email, password]);
  if (isExist.rows.length) {
    res.status(200).send({ message: obj1.reponse1 });
  } else {
    res.status(400).send({ message: obj1.reponse2 });
  }
};

const getloginsociete = async (req, res) => {
  const { email, password } = req.body;
  // check if email and password  exists
  const isExist = await pool.query(queries.checkloginsociete, [
    email,
    password,
  ]);
  if (isExist.rows.length) {
    res.status(200).send(obj1.reponse1);
  } else {
    res.status(400).send(obj1.reponse2);
  }
};
