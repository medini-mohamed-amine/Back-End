...
const updateSociete = (req,res) => {
    const idsociete = parseInt(req.params.idsociete);
    const {nomsociete , adresse, numerotel  } = req.body;
    ...
    // update the societe if he exists in the database
        pool.query(queries.updateSociete, [nomsociete ,adresse ,numerotel, idsociete ] , (error,results) => {
            if (error) throw error;
...
const getloginclient = async (req,res) => {
   ...
   const isExist=  await pool.query(queries.checkloginclient, [email,password] );
    if (isExist.rows.length){
        ...
        res.status(201).send(" email or password incorrect");
        };
    };
...
const getloginsociete = async (req,res) => {
    const {email, password } = req.body;
    // check if email and password  exists
   const isExist=  await pool.query(queries.checkloginsociete, [email,password] );
    if (isExist.rows.length){
        res.send("login  successfully.");
    }else{ 
        res.status(201).send(" email or password incorrect ");
